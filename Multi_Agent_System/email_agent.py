...
def _extract_first_json(text: str) -> str:
    start = text.find("{") # JSON Object Extraction
    if start == -1:
        return ""
    depth = 0
...
def email_agent(task_details: str | dict, context: str = "") -> dict: #Return {to, subject, body} or {error}
    ...
    person = find_email(task_str, interactive=True) #Note to self, Interactivity needs to fixed here, Project asks for two confirmations
    if not person:
...
JSON:"""
    ...
    #override 'to' with the verified address
...
def _get_inbox_for_account(email_address: str):
...
def read_outlook_emails(
    ...
    summarize: bool = True, #Needs more fixes
) -> list[dict]:
    
    try:
        inbox = _get_inbox_for_account(account)
    ...
    if summarize and emails: #Summary Module apparently can't count either
...
def display_inbox(emails: list[dict], batch_summary: bool = True) -> None: #Batch Summaries needs a rework

    if not emails:
        print("\n  No emails to display.")
        return

    if batch_summary and len(emails) > 1: #Could honestly be a function or sumn
...
def scan_unread_emails() -> list[dict]: 
    try:
        ...
        for msg in messages:
            try:
                if msg.UnRead: #NIKO LETS GO BOWLING
                    emails.append({
                        "from":    msg.SenderName,
                        "email":   msg.SenderEmailAddress,
                        "subject": msg.Subject,
