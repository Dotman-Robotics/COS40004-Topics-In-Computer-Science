...
CSV_PATH        = "sample-data.csv" #Need larger dataset for later use
...
def _cosine_rank_candidates(query: str, candidates: list) -> list:
    ...
    return ranked  # List of (score, doc, name)
...
def find_email(query: str, interactive: bool = True) -> dict | None: #Chroma Retrives - Cosine Sim - Confirmation if Necessary (Search String, Interactivity Check)
    ...
    if not interactive: # SKIP THE WAIIIIIIIIIT
    ...
    print(f"\n[vector] Found {len(ranked)} candidate(s) for '{query}':") #LONG WAY ROUND
    for i, (score, doc, name) in enumerate(ranked):
        parts = doc.page_content.split()
        if len(parts) < 2: #Only one match
            continue
        email = parts[-1]
        ...
        if confirmed:
            ...
            if i < len(ranked) - 1:
                ...
                print(f"  No more candidates. Contact not found.")

    return None

# Testing Function, Not being used atm
'''def find_emails_multi(query: str, k: int = 3) -> list[dict]:

    if not query or not query.strip():
        return []

    multi_retriever = vector_store.as_retriever(
        search_kwargs={"k": min(k * 2, 10)}
    )
    candidates = multi_retriever.invoke(query.strip())
    if not candidates:
        return []

    ranked = _cosine_rank_candidates(query.strip(), candidates)[:k]

    results = []
    for score, doc, name in ranked:
        parts = doc.page_content.split()
        if len(parts) < 2:
            continue
        results.append({
